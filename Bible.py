...
class Bible:
    ...
    def _load_book_names(self):
        ...
        for j in range(66):
            self.abbrev2full[self.book_abbr_names[j]] = self.book_full_names[j]
    ...
    def _load_bible(self):
        ...
        for abbr in self.book_abbr_names:
            ...
            with open(os.path.join(self.path_data, f"{abbr}.txt"), "rt") as fin:
                line = fin.read()
                data = line.strip().split("~\n")
                verses = [i.replace('~', '.') for i in data]
            ...
            for verse in verses:
                ...
                ver = verse.split("|")
                value = ver[3].strip()
                book_name, chapter_number, verse_number = ver[0], int(ver[1]), int(ver[2])
    ...
    def search(self, book_full_name, chapter_num, verse_num):
        ...
        abb_fu = []
        for bo_ful in self.full2abbrev:
            if bo_ful == book_full_name:
                ab = self.full2abbrev[bo_ful]
                abb_fu.append(ab)
        value = (abb_fu[-1], chapter_num, verse_num)
        if value in self.bible:
            text = self.bible[value]
        else:
            return None
